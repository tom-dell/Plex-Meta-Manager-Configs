...
Collections are built around a list of movies/shows, these lists can come from Trakt, IMDB, and many other sources, today we will be covering how to make collections using IMDb, and Plex's inbuilt search function.
...
For the collection_order, we generally use 'custom', as it will order the movies, in the same order as the list, or by the conditions in the builder indentation block.
...
Now for the last section, this section is called the builder (we'll define that in a second).
...
To find a poster for the collection, check out [The PosterDB](https://theposterdb.com/). Use the list linked above for this example.
...
First thing first, let's copy our template, without the IMDb fields, add the name, sort_title, summary, and url_poster.
...
Let's sort the collection, with the best movies at the top, but how do we do this? Luckily for us, Plex Meta Manager is very well documented, so let's see what the [sort_option values are in the documentation](https://metamanager.wiki/en/latest/metadata/builders/plex.html#sort-options).
